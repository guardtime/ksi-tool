 ...
 * be detected - see #PARAM_SET_isFormatOK. If parameter name does not exist,
...
/**
 * Extract value attributes with the given constraints.
 * \param	set			#PARAM_SET object.
 * \param	name		Parameter name.
 * \param	source		Constraint for the source, can be \c NULL.
 * \param	priority	Priority that can be #PST_PRIORITY_VALID_BASE (<tt>0</tt>) or higher.
...
/**
 * This function extracts parameter print name that is also displayed in (error)
 ...
 * \param	set			#PARAM_SET object.
 * \param	name		Parameter alias name.
 * \param	print_name	Pointer to receiving pointer.
 * \return #PST_OK when successful, error code otherwise. Some more common error
 ...
 * parameters have at least one value set. Even if the value format or content is
 ...
 * of the parameters has at least one value set. Even if the value format or
...
/**
 * Controls if the format and content of the parameter is OK.
 * \param	set		#PARAM_SET object.
 ...
 * to each set do not affect the other one.
 *
