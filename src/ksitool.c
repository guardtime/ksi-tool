...
static void GT_pritHelp(void){
	...
	print_info(
			...
			" -v --verify\tverify signature or publications file. Use -f or -F\n"
			"\t\tfor document or its hash verification (-n -r -d -t):\n"
...
//			"\t\t--aggre --setsystime set system time from current aggregation.\n"
			...
			" -T <UTC>\tspecify a publication time to extend to (use with -x) as the\n"
