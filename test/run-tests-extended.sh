...
okx src/ksitool -s -f ${SCRIPT_DIR}/testFile -o ${tmp}/tmp.ksig  --log ${tmp}/out.log
like "`[ -f ${tmp}/out.log ] && echo 'Log file exists' || echo 'Log file does not exist' 2>&1`" "Log file exists"
diag "------------------------------------------------------------------------------";
...
okx src/ksitool -v -i ${tmp}/sha1.ksig -f ${SCRIPT_DIR}/testFile -b ${tmp}/pub.bin -V $url_c
...
okx src/ksitool -x -i ${tlv_dir}/ok-sig-2014-08-01.1.ksig -o ${tmp}/ext.ksig -V $url_c --log ${tmp}/out.log
like "`[ -f ${tmp}/out.log ] && echo 'Log file exists' || echo 'Log file does not exist' 2>&1`" "Log file exists"

diag "######    Extend old signature before publication time [-T] "
okx src/ksitool -x -i ${tlv_dir}/ok-sig-2014-08-01.1.ksig -o ${tmp}/ext-t.ksig -T 1413120674 #  (GMT): Sun, 12 Oct 2014 13:31:14 GMT
